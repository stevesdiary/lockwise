...
const ESTATES = [
  { name: 'Greenview Estate',  city: 'Lagos',         state: 'Lagos',  country: 'Nigeria', type: 'residential' },
  { name: 'Sunrise Court',     city: 'Abuja',         state: 'FCT',    country: 'Nigeria', type: 'residential' },
  { name: 'Palm Gardens',      city: 'Port Harcourt', state: 'Rivers', country: 'Nigeria', type: 'mixed'        },
  { name: 'Royal Heights',     city: 'Ikeja',         state: 'Lagos',  country: 'Nigeria', type: 'residential' },
  { name: 'Cedar Park',        city: 'Ibadan',        state: 'Oyo',    country: 'Nigeria', type: 'residential' },
];
...
module.exports = {
  up: async (queryInterface) => {
    ...
    if (!managerRoleId || !residentRoleId) {
      throw new Error('Required roles (manager, resident) not found. Run role seeds first.');
    }
    ...
    const residentUserIds = ESTATES.map(() => [randomUUID(), randomUUID(), randomUUID()]);
    const residentRecordIds = ESTATES.map(() => [randomUUID(), randomUUID(), randomUUID()]);
    ...
    const estateRows = ESTATES.map((estate, i) => ({
      ...
      approved_on:              now,
      setup_checklist:          JSON.stringify({ gates_configured: false, residents_invited: false }),
    ...
    estateRows.forEach((row, i) => { row.estate_id = estateIds[i]; });
    ...
    console.log('✓ Seeded 5 estates, 5 managers, 15 residents');
    console.log('\nCredential summary written to: lockwise-server/docs/demo-credentials.xlsx');
  },

  down: async (queryInterface) => {
    // Remove by email pattern to keep it safe
    await queryInterface.sequelize.query(
