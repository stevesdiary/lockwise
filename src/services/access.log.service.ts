...
class AccessLogService {
  ...
  async approveAccess(accessId: string, approvedBy: string) {
    ...
    const accessCodeTTL = calculateAccessCodeTTL(accessLog.scheduled_entry_end || accessLog.scheduled_entry_date);
    const exitCodeTTL = calculateExitCodeTTL(accessLog.scheduled_exit_end || accessLog.scheduled_exit_date);
  ...
  async processCodeScan(code: string, gateId?: string, scannedBy?: string) {
    ...
    const accessIdFromRedis = await getFromRedis(`access_code:${code}`) || 
    ...
    if (isExitCode && accessLog.actual_entry_time && !accessLog.actual_exit_time) {
    ...
    if (isAccessCode && accessLog.actual_entry_time && !accessLog.actual_exit_time) {
  ...
  async generateResidentExitCode(accessId: string, residentId: string) {
    ...
    const ttl = calculateExitCodeTTL(accessLog.scheduled_exit_end || accessLog.scheduled_exit_date);
    ...
    return { exit_code: newExitCode, expires_at: new Date(Date.now() + ttl * 1000) };
  }
}
