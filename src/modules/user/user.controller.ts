...
const UserController = {
  ...
  register: async (req: TypedRequest, res: Response): Promise<Response> => {
    try {
      ...
      const existingUser = await new UserRepository().findUserByEmail(userCreationData.email, userCreationData.estate_id);
      if (existingUser) {
  ...
  verifyUser: async (req: ExpressRequest, res: Response ): Promise<Response> => {
    try {
      ...
      const verificationResult = await userService.verifyUser({ email, code });
