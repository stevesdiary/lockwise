...
// import { delete, getAllUsersByEstate, getOneUser, updateUser, verifyUser, registerUser, resendCode } from '../repositories/user.repository';
  ...
  register: async (req: TypedRequest, res: Response): Promise<Response> => {
    try {
      ...
      return res.status(user.statusCode).send({
        status: user.status
        message: user.message + (user.statusCode === 201 ? ' Verification code has been sent to your registered email. Check your email' : ''),
        data: user.data
      });
    } catch (error: unknown) {
